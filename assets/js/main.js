          ...
          if (panelsContainers?.length) {
            mediaMatch.add("(min-width: 992px)", () => {
              panelsContainers.forEach((panelsContainer, idx) => {
                const panels = panelsContainer.querySelectorAll(
                  ".invers-arrange-item",
                );
    ...
    academicResourceLibrary: function () {
      $(document).ready(function () {
        ...
        function showToast(title, message, type) {
          ...
          const iconClass =
            type === "download" ? "feather-check" : "feather-lock";
          toast.innerHTML = `
        ...
        document.querySelectorAll(".resource-btn-download").forEach((btn) => {
          btn.addEventListener("click", function (e) {
            ...
            showToast(
              "Descarga iniciada",
              `Iniciando la descarga de "${title}".`,
              "download",
            );
          });
        });
        ...
        document.querySelectorAll(".resource-btn-locked").forEach((btn) => {
          btn.addEventListener("click", function (e) {
            ...
            showToast(
              "Contenido Exclusivo",
              `Este recurso requiere membresía ${required} activa para su visualización y descarga.`,
              "locked",
            );
          });
        });
...
document.addEventListener("DOMContentLoaded", function () {
  // Solo ejecutamos esto si estamos en la página que tiene la grilla
  const grid = document.getElementById("resource-grid");
  const pagination = document.getElementById("resource-pagination");

  // Si no existen en esta página, detenemos el código aquí
  ...
  (function () {
    const resources = [
      {
        badge: "Taller",
        date: "15 Jul 2026",
        title: "Taller: Incoterms 2024 y su aplicación práctica",
        desc: "Aprende a aplicar los Incoterms vigentes en operaciones de comercio exterior con casos reales de empresas peruanas...",
        img: "assets/images/services/serviice-01.jpg",
        link: "service-details-white.html",
      },
      {
        badge: "Oportunidad",
        date: "10 Jul 2026",
        title: "Convocatoria: Programa de Internacionalización 2026",
        desc: "Grupo NI abre convocatoria para estudiantes interesados en representar a su universidad en ferias internacionales.",
        img: "assets/images/services/serviice-02.jpg",
        link: "service-details-white.html",
      },
      {
        badge: "Evento",
        date: "5 Jul 2026",
        title:
          "Ponente invitada: Dra. Lucía Mendoza — Finanzas Internacionales",
        desc: "La experta en banca multilateral y comercio exterior conversará sobre el impacto del tipo de cambio en las...",
        img: "assets/images/services/serviice-03.jpg",
        link: "service-details-white.html",
      },
      {
        badge: "Comunicado",
        date: "1 Jul 2026",
        title: "Comunicado: Actualización de la plataforma Zenit",
        desc: "Hemos mejorado la sección de recursos y certificados. Ahora puedes filtrar por categoría y descargar en un solo clic.",
        img: "assets/images/services/serviice-04.jpg",
        link: "service-details-white.html",
      },
      {
        badge: "Recurso",
        date: "28 Jun 2026",
        title: "Recursos gratuitos: Guía de exportación para MIPYMES",
        desc: "Descarga la guía completa elaborada por nuestros especialistas para iniciar procesos de exportación desde el...",
        img: "assets/images/services/serviice-01.jpg",
        link: "service-details-white.html",
      },
      {
        ...
        date: "20 Jun 2026",
        title:
          "Fecha importante: Cierre de inscripciones al taller de logística",
        desc: "El 22 de julio cierra el plazo de inscripción al taller de Gestión Logística Internacional. ¡No te quedes sin tu lugar!",
        img: "assets/images/services/serviice-02.jpg",
        link: "service-details-white.html",
      },
      {
        badge: "Taller",
        date: "12 Jun 2026",
        title: "Taller: Documentación aduanera esencial",
        desc: "Domina los documentos clave que toda operación de comercio exterior necesita: factura, BL, certificado de origen.",
        img: "assets/images/services/serviice-03.jpg",
        link: "service-details-white.html",
      },
      {
        badge: "Evento",
        date: "3 Jun 2026",
        title: "Webinar: Tendencias del comercio digital en LATAM",
        desc: "Un panel de expertos analizará cómo la digitalización está transformando las cadenas de suministro regionales.",
        img: "assets/images/services/serviice-04.jpg",
        link: "service-details-white.html",
      },
    ];
    ...
    // Plantilla de la tarjeta: Se eliminó 'data-sal' para que se muestre instantáneamente
    function cardTemplate(item) {
      return `
                        <div class="col-xl-4 col-lg-6 col-md-6 col-sm-6 col-12">
                            <div class="card-box card-style-1 text-left tmponhover">
                                <div class="inner">
                                    <div class="image invers-anime">
                                        <span class="tag-news radius-round bg-color-white color-black">${item.badge}</span>
                                        <a href="${item.link}">
                                            <img loading="lazy" src="${item.img}" alt="${item.title}">
                                        </a>
                                    </div>
                                    <div class="content">
                                        <ul class="tmp-meta-list mb--10">
                                            <li class="color-primary w-500">
                                        ...
                                        <div class="discover-btn mt--20">
                                            <a class="tmp-btn btn-small btn-border round" href="${item.link}">
    ...
    function renderPage(page, dataset = resources) {
      const totalPages = Math.max(
        1,
        Math.ceil(dataset.length / ITEMS_PER_PAGE),
      );
      currentPage = Math.min(Math.max(1, page), totalPages);

      const start = (currentPage - 1) * ITEMS_PER_PAGE;
      const pageItems = dataset.slice(start, start + ITEMS_PER_PAGE);

      grid.innerHTML = pageItems.length
        ? pageItems.map(cardTemplate).join("")
        : `<div class="col-12 text-center"><p>No se encontraron recursos.</p></div>`;

      renderPagination(totalPages, dataset);

      // Vuelve arriba de la grilla al cambiar de página
      grid.scrollIntoView({ behavior: "smooth", block: "start" });
    }

    function renderPagination(totalPages, dataset) {
      if (totalPages <= 1) {
      ...
      // Flecha anterior
      html += `<a href="#" class="custom-btn-page ${currentPage === 1 ? "disabled" : ""}" data-page="${currentPage - 1}">
                                    <i class="feather-chevron-left"></i>
                                </a>`;

      // Números de página
      for (let i = 1; i <= totalPages; i++) {
        html += `<a href="#" class="custom-btn-page ${i === currentPage ? "active" : ""}" data-page="${i}">
                                        ${i}
                                    </a>`;
      }

      // Flecha siguiente
      html += `<a href="#" class="custom-btn-page ${currentPage === totalPages ? "disabled" : ""}" data-page="${currentPage + 1}">
                                    <i class="feather-chevron-right"></i>
                                </a>`;

      pagination.innerHTML = html;

      // Nuevo detector de eventos con la nueva clase
      pagination.querySelectorAll("a.custom-btn-page").forEach((a) => {
    ...
    window.renderResourcePage = renderPage;
